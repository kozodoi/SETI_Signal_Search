...
def get_augs(CFG, image_size = None, p_aug = None):
    ...
    # normalization
    if CFG['normalize'] == 'layer' or CFG['normalize'] == 'max' or not CFG['normalize']:
        p_norm = 0
    else:
        p_norm = 1
    CFG['pixel_mean'] = -0.0001
    CFG['pixel_std']  = 0.9055
    
    # train augmentations
    train_augs = A.Compose([A.Resize(p       = 1 - p_crop,
                                     height  = image_size, 
                                     width   = image_size),
                            ...
                            A.Transpose(p      = CFG['p_flip']),
                            A.HorizontalFlip(p = CFG['p_flip']),
                            A.VerticalFlip(p   = CFG['p_flip']),
